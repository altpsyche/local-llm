...
"@ | Set-Content $envFile -Encoding utf8
  ...
  'setup' {
    ...
    if ($setupSub -ne 'check') { Write-Host "Usage: bob setup check"; break }

    $bobCfg = Get-BobConfig
    $venvPy = Join-Path $repo 'tools\venv-litellm\Scripts\python.exe'
    $port   = $bobCfg.litellmPort ?? 8081
    ...
    Write-Host "`nBob agent setup check" -ForegroundColor Cyan
    Write-Host "─────────────────────────────────────────" -ForegroundColor DarkGray
    ...
    $searxPort = $bobCfg.searxngPort ?? 8888
    $searxOk = try {
    ...
    # 8. n8n
    $n8nOk = try { (Invoke-WebRequest "http://localhost:$($bobCfg.n8nPort ?? 5678)" -TimeoutSec 8 -UseBasicParsing -ErrorAction Stop).StatusCode -lt 500 } catch { $false }
    Show-Check "n8n reachable (:$($bobCfg.n8nPort ?? 5678))" $n8nOk 'bob services start'

    # 9. LiteLLM proxy — TCP connect is reliable (health endpoint does backend checks = slow)
    $litellmOk = try { Test-NetConnection -ComputerName localhost -Port $port -InformationLevel Quiet -ErrorAction Stop } catch { $false }
    Show-Check "LiteLLM proxy (:$port)" $litellmOk 'bob litellm'
    ...
    # 12. Tool files present for each enabled tool
    $toolsOk = $true
    foreach ($toolName in ($bobCfg.agent.tools ?? @())) {
      $toolFile = Join-Path $repo "scripts\tools\$toolName.py"
      if (-not (Test-Path $toolFile)) {
        Show-Check "tool: $toolName.py" $false "missing in scripts\tools\"
        $toolsOk = $false
      }
    }
    if ($toolsOk) { Show-Check 'Agent tool files present' $true '' }
  ...
  'agent' {
    ...
    switch ($sub) {
      ...
      'tools' {
        $enabledTools = ($bobCfg.agent.tools ?? @('memory','web','git','file')) -join ','
        $env:PYTHONIOENCODING = 'utf-8'
        & $venvPy "$repo\scripts\tools\tool_loader.py" --list --enabled $enabledTools
        $env:PYTHONIOENCODING = $null
      }
      ...
      'serve' {
        $aPort = $bobCfg.agent.agentPort ?? 8084
        Write-Host "Bob agent HTTP server on :$aPort  (POST /v1/agent/completions)" -ForegroundColor Cyan
        $env:PYTHONIOENCODING = 'utf-8'
        & $venvPy -m uvicorn bob_agent_server:app --host 0.0.0.0 --port $aPort --app-dir "$repo\scripts"
        $env:PYTHONIOENCODING = $null
      }
