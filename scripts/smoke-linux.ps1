#requires -Version 7
# NC7 — the "reliable working Bob" end-to-end smoke. Named for Linux (it's the gate ND promotes into
# the release matrix on a real Linux+CUDA/CPU box) but OS-agnostic in mechanism — it exercises the
# RUNNING stack, so it also passes on Windows when the stack is up.
#
...
#
#   ./scripts/smoke-linux.ps1           # test whatever is already running; SKIP (exit 0) if nothing is up
#   ./scripts/smoke-linux.ps1 -Up       # bring the stack + agent server up first, tear the server down after
