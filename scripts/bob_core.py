...
# M6 — single source of truth for service-port defaults on the Python side. Mirrors
# $script:BobPortDefaults in scripts/_models.ps1. config.json (written by Get-BobConfig)
...
def get_role(config: dict, task: str = "chat", pro: bool = False) -> str:
    ...
    Centralizes the routing lookup so the plugins don't each re-derive it.
    """
    if task == "vision":
        vision = config.get("vision", {})
        return vision.get("visionProRole", "vision-pro") if pro else vision.get("visionRole", "vision")
    routing = config.get("routing", {})
    # task -> (base routing key, pro routing key, hard default)
    table = {
        "chat":  ("defaultRole", "proRole",      "chat"),
        "voice": ("defaultRole", "proRole",      "chat"),
        "code":  ("codeRole",    "proCodeRole",  "coder"),
        "think": ("thinkRole",   "proThinkRole", "planner"),
        "agent": ("agentRole",   "agentRole",    "agent"),
    }
    base_key, pro_key, default = table.get(task, ("defaultRole", "proRole", "chat"))
    if pro:
        return routing.get(pro_key) or routing.get(base_key) or default
    return routing.get(base_key) or default
