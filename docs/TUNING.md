...
Three config files (and the Open WebUI model table) are generated automatically from [config/models.psd1](../config/models.psd1) every time you run `llm serve` or `llm gen`. Don't edit them by hand; edit the PSD1 file instead.
...
`llm gen` calls `scripts/gen-webui.ps1`, which writes these into the Open WebUI SQLite database. Override in `config/user.psd1` using the same key names.
