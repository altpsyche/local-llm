...
#      bob profile 12gb           (persists + regenerates the runtime config)
#      setup.bat -Profile 12gb    (at setup time)
#  Peek a profile without switching or downloading:
#      bob fetch --list 12gb      (dry-run; lists files + sizes, pulls nothing)
#
...
#  matters. Quick check:  bob fetch --list <profile>  then HEAD the resolve URLs.
# =============================================================================
@{
  # The active profile. '$env:BOB_PROFILE' (if set) overrides this for one
  ...
  # Re-run `bob gen` after editing user.psd1. Changes take effect on next `bob serve`.
  defaults = @{
  ...
  # Do not edit these; they are overwritten at every `bob gen` / `bob serve`.
  macros = @{
  ...
  # Re-run `bob gen` after editing. No llama-swap restart needed.
  #
  ...
  profiles = @{
    ...
    # Still swaps big models. UNVALIDATED: no 24GB card in authors' hardware — verify with bob bench.
    # Repo/path URLs need verification on huggingface.co before first use (bob fetch --list 24gb).
    '24gb' = @{
    ...
    # UNVALIDATED: verify with bob bench. If planner OOMs, lower ctx or add '-ngl' flag.
    # Repo/path URLs need verification on huggingface.co before first use (bob fetch --list 32gb).
    '32gb' = @{
    ...
    # until it fits. UNVALIDATED: no 8GB card in the authors' hardware; verify with bob bench.
    # Repo/path URLs need verification on huggingface.co before first use (bob fetch --list 8gb).
    '8gb' = @{
