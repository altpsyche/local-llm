...
@{
  ...
  peers = @{
    deepseek = @{
      ...
      pro = @{
        chat    = 'deepseek-chat'                  # DeepSeek V3  — ~$0.27/M in, $1.10/M out
        planner = 'deepseek-reasoner'              # DeepSeek R1  — ~$0.55/M in, $2.19/M out
      }
    }
    zhipu = @{
      enabled       = $true
      proxy         = 'https://open.bigmodel.cn/api/paas/v4'
      apiKeyEnv     = 'ZHIPU_API_KEY'              # env var NAME — never the key value
      litellmPrefix = 'openai'                     # OpenAI-compatible; api_base from proxy
      pro = @{
        coder = 'glm-4-flash'                      # GLM-4-Flash  — very cheap
      }
    }
  }
